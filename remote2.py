...
class RemoteContainer(object):
    def __init__(self, name):
        ...
        if self.region == 'dfw':
            ...
            self.container_name = '%s/%s' %(self.region, name)
...
class RemoteDirectory(object):
    def __init__(self, region, container_name, path):
        ...
        if not self.path:
            ...
            self.shalist_path = os.path.join(self.path, '.shalist')
            prefix = self.path

        for i in range(3):
            try:
                self.shalist = json.loads(container.get_object(
                        self.shalist_path).fetch())
                break
            except:
                pass
    ...
    def write_shalist(self):
        shafile = os.path.join(self.path, '.shalist')
...
class RemoteFile(object):
    ...
    def size(self):
        ...
        conn = pyrax.connect_to_cloudfiles(region=self.region.upper())
        container = conn.create_container(self.container_name)
        obj = container.get_object(self.path)
    ...
    def write_checksum(self, checksum):
        ...
        shafile = os.path.join(self.container_path, '.shalist')
        print '%s Updating  %s with %s' %(datetime.datetime.now(), shafile,
                                          self.path)
    ...
    def fetch(self):
        ...
        url = container.get_object(self.path).get_temp_url(3600)
        url = url.replace(' ', '%20')
